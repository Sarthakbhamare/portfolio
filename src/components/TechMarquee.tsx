    ...
    <div className="flex overflow-hidden py-2">
      ...
      >
        {duplicatedItems.map((tech, index) => (
          <span
            key={`${tech}-${index}`}
            className="flex-shrink-0 px-4 py-2 rounded-full glass text-sm text-muted-foreground hover:text-foreground hover:bg-secondary/50 transition-all duration-300 cursor-default"
          >
            {tech}
          </span>
        ))}
  ...
  return (
    <section className="py-16 overflow-hidden">
      <div className="space-y-3">
        <MarqueeRow items={techStack1} speed={35} />
        <MarqueeRow items={techStack2} reverse speed={40} />
        <MarqueeRow items={techStack3} speed={30} />
      </div>
    </section>
  );
};
